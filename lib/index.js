...
const getAddress = (options, req) => {
  ...
  if (address === '::1') {
    ...
    if (result && result[2]) address = result[2];
...
const _handle = function (options, table, req) {
  return new Promise((resolve, reject) => {
    ...
    if (options.testmode) {
      //console.log('ddos defend is beginning!!!', table);
    }
    ...
    if (options.whitelist.indexOf(host) != -1) {
      console.log('The ip address in the whitlist!');
      return resolve();
    }
    ...
    if (!table[host]) table[host] = { count: 1, expiry: 1 };
      ...
      if (
        table[host].count > options.burst &&
        table[host].expiry <= options.maxexpiry
      ) {
    ...
    if (table[host].count > options.limit) {
      ...
      if (options.testmode) {
        return reject({
          action: 'respond',
          code: 429,
          message: JSON.stringify(table[host]),
        });
      } else {
        return reject({
          action: 'respond',
          code: options.responseStatus,
